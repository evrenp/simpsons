...
DATA_PATH = os.path.join(os.getenv('HOME'), 'data', 'simpsons')
PIC_PATH = os.path.join(DATA_PATH, 'the-simpsons-characters-dataset', 'simpsons_dataset')
...
def load_pictures(BGR):
    ...
    for k, char in map_characters.items():
        pictures = [k for k in glob.glob(str(PIC_PATH) + '/%s/*' % char)]
...
def get_dataset(save=False, load=False, BGR=False):
    ...
    """
    if load:
        h5f = h5py.File(DATA_PATH + '/dataset.h5', 'r')
        X_train = h5f['X_train'][:]
        X_test = h5f['X_test'][:]
        h5f.close()

        h5f = h5py.File(DATA_PATH + '/labels.h5', 'r')
        y_train = h5f['y_train'][:]
        y_test = h5f['y_test'][:]
        h5f.close()
    else:
        ...
        if save:
            h5f = h5py.File(DATA_PATH + '/dataset.h5', 'w')
            h5f.create_dataset('X_train', data=X_train)
            h5f.create_dataset('X_test', data=X_test)
            h5f.close()

            h5f = h5py.File(DATA_PATH + '/labels.h5', 'w')
...
def training(model, X_train, X_test, y_train, y_test, data_augmentation=True, n_epochs=40, steps_per_epoch=10,
             model_id='bla'):
    """
    Training.
    ...
    if data_augmentation:
        ...
        datagen.fit(X_train)
        filepath = os.path.join(DATA_PATH, 'weights_{}.hdf5'.format(model_id))
...
if __name__ == '__main__':
    X_train, X_test, y_train, y_test = get_dataset(load=True, save=False)
    ...
    model, history = training(model, X_train, X_test, y_train, y_test, data_augmentation=True, n_epochs=50,
                              steps_per_epoch=steps_per_epoch, model_id=model_id)

    # save history_dict
    history_dict = {key: getattr(history, key) for key in ['history', 'epoch', 'params']}
    with open(os.path.join(DATA_PATH, 'history_{}.pkl'.format(model_id)), "wb") as handle:
