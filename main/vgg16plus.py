...
#     data = load_data(data_id=data_id)
#
#     with open(os.path.join(model_path, '{}_vgg16_features_train.pkl'.format(data_id)), 'rb') as f:
#         features_train = pickle.load(f)
#
#     with open(os.path.join(model_path, '{}_vgg16_features_test.pkl'.format(data_id)), 'rb') as f:
#         features_test = pickle.load(f)
#
#     model = Sequential()
...
#
# def predict_plus(data_id='medium', data_path=DATA_PATH, model_path=MODEL_PATH):
#     with open(os.path.join(model_path, '{}_vgg16_features_test.pkl'.format(data_id)), 'rb') as f:
#         features_test = pickle.load(f)
#
#     data = load_data(data_id=data_id)
...
#                   loss='binary_crossentropy', metrics=['accuracy'])
#
#     model.load_weights(os.path.join(MODEL_PATH, 'some_exp.hdf5'))
#
#     y_hat = model.predict(features_test)
#     labels_num_test_hat = data.label_binarizer.inverse_transform(y_hat)
#     from sklearn.metrics import classification_report
#     print(classification_report(y_true=data.labels_num_test, y_pred=labels_num_test_hat, digits=2))
#
#
